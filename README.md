...
- 水质风险：录入溶解氧、水温、pH、氨氮和浊度，自动判定正常/预警/严重。预警读数单级确认即生效；严重读数实行双人复核闭环，两名操作员都通过后才生效。
...
2. 在“水质读数”录入当前指标；预警读数由一名操作员单级确认；严重读数先由首名操作员现场核实并生成待复核记录，再由另一名操作员第二级复核，通过才生效（否决则保持严重并写明原因）。
3. 在“投喂计划”创建草稿并提交，主管检查最新水质后批准。严重读数双人复核未闭环（待复核/已否决）时，既不能批准计划也不能安排投喂；系统只认最新读数，不会退回更早读数放行。
...
- Go：`backend/internal/constants/enums.go`
  ...
  - `RiskLevel`、`ReadingReview`（`not_required` / `unverified` / `pending_review` / `approved` / `rejected`）、`ExecutionStatus`、`Role`
...
| `PATCH` | `/api/readings/:id/confirm` | 预警单级确认；严重读数首名操作员核实送审（不生效） |
| `PATCH` | `/api/readings/:id/review` | 严重读数第二级复核：`{"approved":true/false,"note":"..."}`，另一名操作员通过才生效 |
...
- 正常读数无需确认；预警读数保持单级确认，确认后即生效。
- 严重读数必须双人复核闭环：首名操作员核实后只生成待复核记录，读数保持严重且不生效，不能据此批准计划或安排投喂；由另一名操作员第二级复核通过后读数才生效，否决则维持严重并写明原因。
- 严重读数同一人不能同时完成首核与终审；重复或并发复核只会产生一个结果（行锁 + SERIALIZABLE 事务）；被否决后可由首名操作员重新核实送审。
- 放行只认最新读数：严重读数处于待复核或已否决期间，计划批准与投喂安排一律阻断，不会退回更早读数放行。
- 进入双人复核流程（待复核/已否决）的严重读数不允许删除。
