...
- 水质风险：录入溶解氧、水温、pH、氨氮和浊度，自动判定正常/预警/严重。预警读数单级确认即可；严重读数实行双人复核闭环——首名操作员只能核实生成待复核记录，另一名操作员通过后读数才生效，否决则保持严重并写明原因。
...
2. 在“水质读数”录入当前指标；预警读数现场单级确认；严重读数先由首名操作员一级核实生成待复核记录，再由另一名操作员通过或否决。
...
| `PATCH` | `/api/readings/:id/confirm` | 预警单级确认 / 严重一级核实（仅生成待复核记录） |
| `PATCH` | `/api/readings/:id/review/approve` | 另一名操作员二级复核通过，严重读数生效 |
| `PATCH` | `/api/readings/:id/review/reject` | 另一名操作员二级复核否决，保持严重并写明原因 |
...
- 严重水质读数必须双人复核闭环：首名操作员一级核实后只生成待复核记录，不能解除异常，也不能据此批准计划或安排投喂；另一名操作员通过后读数才生效，否决则保持严重并写明原因（至少 10 字）。
- 同一人不得完成同一严重读数的两级；重复或并发复核只能产生一个结果；否决后处置完毕可重新核实，再由不同的人复核。
- 待复核期间以该严重读数为准，不得退回更早读数放行投喂或批准；正常、预警读数的单级确认和现有启动方式保持不变。
- 关闭养殖池不接受新水质读数或投喂计划。
